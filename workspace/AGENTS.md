...
**绝对不允许**只用文字说"我派了 T-xxx"但没有实际调用 dispatch_worker。

正确流程：
1. 调 `dispatch_worker` 工具 → 拿到 task_id / run_id
2. 用拿到的真实 ID 告诉 Cai
...
- Worker 完成/失败 → 系统会通知你，附带报告路径
- **必须 `read_file` 读完整报告**，不要只看通知摘要
- 成功 → 提取关键发现，带证据（文件路径+代码）汇报给 Cai
- 失败 → 分析原因，补充 context 后重试，或自己做
